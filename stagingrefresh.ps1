...
#Copy database environment: Staging
# This script will clone the production environment down to the staging environment: 
# It will clone the master and then the slave
# Usage stagingrefresh.ps1
#
...
$vcpass = ")LOA8C^ruZJ)"
$LogFile = "C:\Users\admin_llacroix\stagingrefresh.log"
...
# New vms:
$targetmaster = "stmt-dbv03"
$targetslave = "stmt-dbv04"
...
$appserver1 = "stmt-apv01"
$appserver2 = "stmt-apv02"
$appserver3 = "stmt-apv03"
$NetworkName = "prod-rdb-db"
$IPADDRmaster = "10.2.20.27"
$IPADDRslave = "10.2.20.28"
$GATEWAY = "10.2.20.1"
$environment = "staging"
#Enable logging:
...
Invoke-VMScript -VM $targetmaster -ScriptText "sed -i -e 's/server-id                       = 1/server-id                       = 11/g' /etc/my.cnf" -GuestUser $GuestUser -GuestPassword $GuestPassword
...
Invoke-VMScript -VM $targetslave -ScriptText "sed -i -e 's/server-id                       = 5/server-id                       = 12/g' /etc/my.cnf" -GuestUser $GuestUser -GuestPassword $GuestPassword
...
$Password = "HQnRqLX3hK"
$database = "reeher_dash_dbo"
$querymaster = "use reeher_dash_dbo; UPDATE reeher_dash_dbo.client SET idp_sso_target_url='https://app.onelogin.com/trust/saml2/http-post/sso/430393' WHERE client_id=3; UPDATE reeher_dash_dbo.client SET idp_sso_target_url='https://app.onelogin.com/trust/saml2/http-post/sso/430383' WHERE client_id=18;"
$ConnectionString = "Server=$targetmaster;uid=$UserName;pwd=$Password;Database=$database;Integrated Security=False;"
$MySQLConnection = New-Object -TypeName MySql.Data.MySqlClient.MySqlConnection
$MySQLConnection.ConnectionString = $ConnectionString
$MySQLConnection.Open()
...
Invoke-VMScript -VM $appserver3 -ScriptText "service tomcat restart" -GuestUser $GuestUser -GuestPassword $GuestPassword 
